...
Parallel.each(['a','b','c'], start: ->(item, index, result) { puts "Start: #{item}, #{index + 1}, #{result}" }) { |item| puts item * 5 }

Parallel.each(['a','b','c'], finish: ->(item, index, result) { puts "Finish: #{item}, #{index + 1}, #{result}" }) { |item| item * 6 }

Parallel.each('a'..'c') { |item| puts item * 7 }

items = [1,2,3]
puts Parallel.each(-> { items.pop || Parallel::Stop }) { |item| item }.call
...
items = [1,2,3]
puts Parallel.each_with_index(-> { items.pop || Parallel::Stop }) { |item, index|
